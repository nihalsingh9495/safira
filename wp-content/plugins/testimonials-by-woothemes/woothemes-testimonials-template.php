...
function woothemes_testimonials ( $args = '' ) {
	...
	do_action( 'woothemes_testimonials_before', $args );
		...
		if ( ! is_wp_error( $query ) && is_array( $query ) && count( $query ) > 0 ) {
			...
			$tpl = '<div class="quote-container"><div id="quote-%%ID%%" class="%%CLASS%%" itemscope itemtype="http://schema.org/Review">%%AVATAR%%<blockquote class="testimonials-text" >%%TEXT%%</blockquote>%%AUTHOR%%</div></div>';
			...
			foreach ( $query as $post ) { $count++;
				...
				if ( ( get_the_title( $post ) != '' ) && true == $args['display_author'] ) {
					$author .= '<cite class="author" itemscope itemtype="http://schema.org/Person">';

					$author_name = '<span class="name">' . get_the_title( $post ) . '</span>';

					$author .= $author_name;

					if ( isset( $post->byline ) && '' != $post->byline ) {
						$author .= ' <span class="title">' . $post->byline . '</span><!--/.title-->' . "\n";
					}

					if ( true == $args['display_url'] && '' != $post->url ) {
						$author .= ' <span class="url"><a href="' . esc_url( $post->url ) . '" >' . apply_filters( 'woothemes_testimonials_author_link_text', $text = esc_url( $post->url ) ) . '</a></span><!--/.excerpt-->' . "\n";
					}
